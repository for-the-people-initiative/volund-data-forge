...
import { generateMigration, generateInverseMigration, MigrationError } from './migration-generator.js';
...
export class MigrationManager {
  ...
  async applySchema(
    ...
    if (!currentSnapshot) {
      ...
      this.logger?.debug('DDL: createCollection', { collection: schema.name, fieldCount: schema.fields.length });
    ...
    this.logger?.info('Applying schema migration', { collection: schema.name, fromVersion: currentVersion, toVersion: nextVersion, changes: diff.changes.length });
    ...
    // Execute migration
    this.logger?.debug('DDL: executeMigration', { collection: schema.name, operations: migration.operations.length });
