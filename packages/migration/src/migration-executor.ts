...
async function executeOperation(adapter: DatabaseAdapter, op: MigrationOperation): Promise<void> {
  switch (op.type) {
    case 'addField':
    case 'addForeignKey':
    ...
    case 'dropCollection':
      ...
      throw new MigrationError(
        `dropCollection not yet supported by adapter. Collection: ${op.collection}`,
      )
