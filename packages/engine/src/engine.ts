...
import { validateQuery } from './query-compiler.js'
...
export class DataEngine {
   ...
   */
  private getReferencingRelations(collection: string): Array<{
    schemaName: string
    fieldName: string
    rel: NonNullable<import('@data-engine/schema').FieldDefinition['relation']>
  }> {
    const result: Array<{
      schemaName: string
      fieldName: string
  ...
  private buildPopulateDefinitions(
    ...
    for (const pop of populate) {
      ...
      if (!field || !field.relation) {
        throw new EngineError(
          `Field "${fieldName}" is not a relation on collection "${schema.name}"`,
          'INVALID_POPULATE',
        )
      }
  ...
  private async populateResults(
    ...
    for (const pop of populate) {
      ...
      if (rel.type === 'manyToOne' || rel.type === 'oneToOne') {
        ...
        const relQuery: QueryAST = {
          filters: { and: [{ field: 'id', operator: 'in', value: ids }] },
        }
        const client = txClient ?? this.adapter
