    ...
    for (const field of query.select) {
      if (!validFields.has(field)) {
        throw new QueryCompilationError(
          `Unknown field "${field}" in select for collection "${schema.name}"`,
        )
      }
    }
  }
  ...
  if (query.sort) {
    for (const clause of query.sort) {
      if (!validFields.has(clause.field)) {
        throw new QueryCompilationError(
          `Unknown field "${clause.field}" in sort for collection "${schema.name}"`,
        )
      }
    }
  }
...
function validateFilterCondition(
  ...
  if (!validFields.has(rootField)) {
    throw new QueryCompilationError(
      `Unknown field "${condition.field}" in filter for collection "${collection}"`,
    )
  }
}
