...
export default defineEventHandler(async (event) => {
  ...
  if (method === 'PUT') {
    ...
    // Validate
    const knownCollections = registry.getAll().map(s => s.name).filter(n => n !== collection);
    ...
    try {
      ...
      if (!mm) {
        ...
        return { error: { code: 'NO_MIGRATION_MANAGER', message: 'Migration manager not available' } };
      }
