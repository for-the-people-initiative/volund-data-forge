...
export default defineEventHandler(async (event) => {
  ...
  if (method === 'POST') {
    ...
    if (!body || !body.name) {
      ...
      return { error: { code: 'INVALID_BODY', message: 'Request body must include a collection schema with a name' } };
    }
    ...
    try {
      ...
      if (mm) {
        ...
        if (!result.success) {
          ...
          return { error: { code: 'MIGRATION_FAILED', message: 'Failed to create collection', details: result } };
        }
      } else {
