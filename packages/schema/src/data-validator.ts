  ...
  switch (rule) {
    case 'min':
      ...
      if (typeof value === 'string' && value.length < (ruleValue as number)) {
        return { path, message: customMessage ?? `"${field}" must be at least ${ruleValue} characters` };
      }
      break;
    case 'max':
      if (typeof value === 'number' && value > (ruleValue as number)) {
      ...
      if (typeof value === 'string' && value.length > (ruleValue as number)) {
        return { path, message: customMessage ?? `"${field}" must be at most ${ruleValue} characters` };
      }
      break;
    case 'pattern':
