...
describe('DataEngine CRUD Integration', () => {
    ...
    let engine;
    beforeAll(async () => {
        adapter = new KnexAdapter({ client: 'better-sqlite3', database: ':memory:', primaryKey: 'uuid' });
    ...
    describe('create', () => {
        ...
        it('throws validation error on invalid type', async () => {
            await expect(engine.create('companies', { name: 123 }))
                .rejects.toThrow(EngineError);
            try {
                await engine.create('companies', { name: 123 });
            }
            catch (e) {
                expect(e.code).toBe('VALIDATION_ERROR');
    ...
    describe('update', () => {
        it('performs partial update and updates updated_at', async () => {
            ...
            const updated = await engine.update('companies', { filters: { and: [{ field: 'id', operator: 'eq', value: created.id }] } }, { name: 'New Name' });
        ...
        it('rejects system field updates (id)', async () => {
            await expect(engine.update('companies', { filters: { and: [{ field: 'id', operator: 'eq', value: 1 }] } }, { id: 999 })).rejects.toThrow(/system field.*id/i);
        });
        it('rejects system field updates (created_at)', async () => {
            await expect(engine.update('companies', { filters: { and: [{ field: 'id', operator: 'eq', value: 1 }] } }, { created_at: 'hack' })).rejects.toThrow(/system field.*created_at/i);
        });
    });
    // ─── DELETE ────────────────────────────────────────────────────
    describe('delete', () => {
    ...
    describe('relations', () => {
        it('populates manyToOne (contact → company)', async () => {
            ...
            expect(contacts[0].company.name).toBe('RelCorp');
        });
    });
    // ─── HOOKS ─────────────────────────────────────────────────────
    describe('hooks', () => {
...
//# sourceMappingURL=crud-engine.test.js.map