...
import { validateSchema, SchemaRegistry, } from '@data-engine/schema';
...
describe('Schema Validation', () => {
    ...
    it('invalid schema: missing name', () => {
        const errors = validateSchema({ name: '', fields: [] });
        expect(errors.length).toBeGreaterThan(0);
        expect(errors.some(e => e.path === 'name')).toBe(true);
    });
    it('invalid schema: bad name format', () => {
        const errors = validateSchema({ name: 'My Posts!', fields: [] });
...
describe('Schema Registry', () => {
    ...
    it('rejects invalid schema', async () => {
        await expect(registry.register({ name: '', fields: [] })).rejects.toThrow('validation failed');
    });
});
...
describe('KnexAdapter DDL + Introspection', () => {
    ...
    it('addField adds a column', async () => {
        const newField = { name: 'subtitle', type: 'text' };
